...
def estimate_aggd_param(block):
    """估计 AGGD（非对称广义高斯分布）参数。

    参数:
        block (ndarray): 二维图像块。

    返回:
        tuple: AGGD 分布的 alpha、beta_l 和 beta_r。
    """
    block = block.flatten()
...
def niqe(img, mu_pris_param, cov_pris_param, gaussian_window, block_size_h=96, block_size_w=96):
    """计算 NIQE（Natural Image Quality Evaluator）指标。

    NIQE 是无参考图像质量评价指标，会把图像局部统计特征与自然图像先验模型比较。

    参数:
        img (ndarray): 待评估图像，必须是灰度图或 Y 通道图，形状为 (H, W)，范围 [0, 255]。
        mu_pris_param (ndarray): 自然图像先验多元高斯模型的均值。
        cov_pris_param (ndarray): 自然图像先验多元高斯模型的协方差。
        gaussian_window (ndarray): 用于平滑图像的 7x7 高斯窗口。
        block_size_h (int): 分块高度。默认值：96。
        block_size_w (int): 分块宽度。默认值：96。
    """
    ...
    for scale in (1, 2):  # perform on two scales (1, 2)
        ...
        if scale == 1:
            img = bicubic(img / 255., scale=0.5, antialiasing=True)
            img = img * 255.
...
def calculate_niqe(img, crop_border, input_order='HWC', convert_to='y', **kwargs):
    ...
    参数:
        img (ndarray): 待评估图像，范围可为 [0, 1] 或 [0, 255]。
        crop_border (int): 每条边裁剪的像素数。
        input_order (str): 输入顺序，支持 'HW'、'HWC' 或 'CHW'。默认值：'HWC'。
        convert_to (str): 转换到 'y' 通道或 'gray' 灰度图。默认值：'y'。

    返回:
        float: NIQE 结果，数值越低通常表示质量越好。
    """
