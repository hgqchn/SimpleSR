...
`UNetModel` 可用于普通 DDPM。
`SuperResModel` 在输入端拼接 ``x_t`` 和上采样后的低分图像 ``lq``，用于超分 DDPM。
"""
...
from diffusion.common_utils import TimestepBlock, TimestepSequential, group_norm32, timestep_embedding, zero_module,conv_nd,linear,avg_pool_nd
from diffusion.self_attention import AttentionBlock_Residual as AttentionBlock
...
class ResBlock(TimestepBlock):
    ...
    def __init__(
        ...
        self.out_layers = nn.Sequential(
            group_norm32(self.out_channels),
            nn.SiLU(),
...
class UNetModel(nn.Module):
    ...
    :param num_res_blocks: number of residual blocks per downsample. It can be
        an int shared by all levels, or a list with one value per level.
    ...
    def __init__(
        ...
        if isinstance(num_res_blocks, int):
            self.num_res_blocks = [num_res_blocks] * len(self.channel_mult)
        else:
            assert len(num_res_blocks) == len(self.channel_mult)
        ...
        self.input_blocks = nn.ModuleList(
            [TimestepSequential(conv_nd(self.dims, self.in_channels, ch, 3, padding=1))]
        )
